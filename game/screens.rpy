...
style say_dialogue_ruby is default
...
style say_label:
    ...
    yalign 0.5
    outlines [(3, "#2a1a0aCC", 0, 0), (1, "#000000AA", 1, 1)]
...
style say_dialogue:
    ...
    outlines [(2, "#1a0f00CC", 0, 0), (1, "#000000AA", 1, 1)]
    line_leading 18
    line_spacing 6
    ruby_line_leading 28
    ruby_style style.say_dialogue_ruby
    adjust_spacing False

style say_dialogue_ruby:
    yoffset -32
    size 24
    font "DejaVuSans.ttf"
    color "#FFD166"
...
style choice_button is default:
    ...
    background Solid("#274D36EE")
    hover_background Solid("#3B7650F5")
    padding (42, 18)
    ...
    if quick_menu:
        ...
        hbox:
            ...
            textbutton _("\u5feb\u5b58"):
                ...
                action FileSave(get_account_quick_slot(), page=get_account_quick_page(), confirm=False)
            textbutton _("\u5feb\u8bfb"):
                sensitive bool(get_current_account()) and story_state_is_stable()
                action FileLoad(get_account_quick_slot(), page=get_account_quick_page(), confirm=True)
...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            textbutton _("\u4fdd\u5b58"):
                sensitive story_state_is_stable()
        ...
        textbutton _("家长报告") action ShowMenu("parent_reports_history")
        ...
        if renpy.variant("pc"):
            ...
            if main_menu:
                ...
                textbutton _("退出") action Show("confirm_save_and_quit")
...
screen main_menu():
    ...
    if persistent.current_user and persistent.current_user in persistent.accounts:
        $ _cur_user_name = persistent.accounts[persistent.current_user].get("name", "")
        $ _cur_is_guest = persistent.accounts[persistent.current_user].get("is_guest", False)
        frame:
            ...
            hbox:
                spacing 8
                if _cur_is_guest:
                    text "游客模式" size 16 color "#87CEEB"
                else:
                    text _cur_user_name size 16 color "#FFD700"
...
screen file_slots(title):
    ...
    $ _page_range = get_user_page_range(persistent.current_user)
    $ _start_page = _page_range[0]
    $ _end_page = _page_range[1]
    $ _current_page_text = FileCurrentPage()
    $ _current_page = int(_current_page_text) if _current_page_text.isdigit() else _start_page

    on "show" action FilePage(_start_page)
        ...
        fixed:
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        sensitive story_state_is_stable() and ((not FileJson(slot)) or FileJson(slot).get("account_key", "") in ("", persistent.current_user))
                        ...
                        # 存档元数据增强（角色、情景、属性条）
                        $ _slot_json = FileJson(slot)
                        if _slot_json and _slot_json.get("character"):
                            $ _sj_char = _slot_json.get("character", "")
                            $ _sj_color = _slot_json.get("character_color", "#888888")
                            $ _sj_scenario = _slot_json.get("scenario", "")
                            $ _sj_avatar = _slot_json.get("character_avatar", "")
                            hbox:
                                spacing 6
                                xalign 0.5
                                if _sj_avatar:
                                    add Transform(_sj_avatar, size=(24, 24)) yalign 0.5
                                text _sj_char size 16 color "#FFF4D6" yalign 0.5
                                if _sj_scenario:
                                    $ _sj_sc_short = _sj_scenario[:10]
                                    text _sj_sc_short size 14 color "#aaaaaa" yalign 0.5
                            if _slot_json.get("stats"):
                                $ _sj_stats = _slot_json["stats"]
                                hbox:
                                    spacing 3
                                    xalign 0.5
                                    for _sn in ["智", "勇", "体", "友"]:
                                        $ _sv = _sj_stats.get(_sn, 5)
                                        $ _sc = {"智": "#3498db", "勇": "#e74c3c", "体": "#2ecc71", "友": "#f39c12"}.get(_sn, "#888")
                                        text _sn size 11 color _sc yalign 0.5
                                        bar:
                                            value _sv
                                            range 10
                                            xsize 35
            ...
            vbox:
                ...
                hbox:
                    ...
                    textbutton _("<"):
                        sensitive _current_page > _start_page
                        action FilePage(max(_start_page, _current_page - 1))
                    key "save_page_prev" action If(_current_page > _start_page, FilePage(_current_page - 1), NullAction())

                    for _abs_page in range(_start_page, _end_page + 1):
                        $ _display_page = _abs_page - _start_page + 1
                        textbutton "[_display_page]" action FilePage(_abs_page)

                    textbutton _(">"):
                        sensitive _current_page < _end_page
                        action FilePage(min(_end_page, _current_page + 1))
                    key "save_page_next" action If(_current_page < _end_page, FilePage(_current_page + 1), NullAction())
    ...
    use game_menu(_("设置"), scroll="viewport"):
        ...
        vbox:
            ...
            hbox:
                ...
                xalign 0.5
                label _("语音朗读")
                textbutton _("开启") action Preference("self voicing", "toggle")
            ...
            hbox:
                ...
                vbox:
                    ...
                    label _("辅助功能")
                    textbutton _("拼音注音"):
                        action ToggleField(persistent, "pinyin_mode")
                        sensitive _pypinyin_available
                    if not _pypinyin_available:
                        text _("拼音数据未加载") size 16 color "#e74c3c"
