...
# 多账户使用各自的存档页。Ren'Py 全局 auto 页无法表达所有权，因此关闭自动存档。
define config.has_autosave = False
define config.rollback_enabled = False

## 使用 Ren'Py 的系统自读，由操作系统选择可用声音。
init python:
    config.tts_voice = ""

    # TTS 文本过滤：剥离 Ruby/拼音标签，只朗读汉字
    config.tts_substitutions = [
        # {rb}汉字{/rb}{rt}...{/rt} → 只保留汉字
        (r'\{rt\}.*?\{/rt\}', ''),
        (r'\{rb\}', ''),
        (r'\{/rb\}', ''),
        # 清除颜色/尺寸标签（以防遗漏）
...
define config.save_directory = "XiongchumoQiyu-1773637272"
...
init python:
    ...
    build.classify('**/assets-source/**', None)
    build.classify('**/archive/**', None)
    ...
    ## 脚本文件：RPA 只是归档格式，不应被视为加密或安全边界。
    ## 发布包保留已编译脚本，源码留在版本库中审查。
